...
(Ollama by default, OpenAI via a one-click mode toggle at runtime).
...
|---|---|---|---|---|
| Cache version has no changelog | `generation.py` `GENERATION_SCHEMA_VERSION` | A blunt full cache-bust with no record of what each bump changed. | Maintain a version→change map. See [ADR-0004](docs/adr/0004-cache-via-schema-version.md). | Low |
| Cache not keyed by provider/model | `generation.py` cache tables | After a mode switch, cached artifacts generated by the *other* mode are served (fine for cost, confusing when comparing local vs hosted quality). | Include provider/model in the cache key, or surface `model_name` in the UI. See [ADR-0010](docs/adr/0010-local-by-default.md). | Low |
| No streaming chat | `llm_client.py`, `frontend/pages/chat.py` | Long local chat answers render only when complete; `OllamaClient.chat_json(stream=True)` raises. | Stream `/api/chat` (no `format` on chat would be needed — schema is `{answer}` only) into the chat panel. | Medium (UX) |
| Attributed-sentence text unverified | `generation.py:_ground_intersections` | Citation grounding checks source *ids* only; the quoted `text` is never matched against the ledger, so a fabricated quote citing two real ids survives. | Fuzzy-match attributed text against ledger claims/evidence. | Low/Medium |

### Recently resolved

- **v2.3 (local-first release)** — local by default with a one-click Local↔API mode toggle and
  per-mode model bundles ([ADR-0010](docs/adr/0010-local-by-default.md)); no import-time key
  requirement; Ollama structured outputs + `num_ctx`/`keep_alive`/batched embed; on-device
  transcription (parakeet-mlx/mlx-whisper) behind a real `whisper_provider` switch; embedding-model-
  keyed Chroma collections with task prefixes (dimension-collision fix); provider-aware concurrency;
  chat cross-source-set context leak fixed; Ollama token telemetry recorded; telemetry writes moved
  out of the LLM retry path + SQLite WAL; fast-fail on 401/403/404; per-stage progress endpoint +
  UI polling; double-submit guard; event-loop-blocking uploads/downloads fixed; duplicate-filename
  clobber fixed; quiz prompts drop evidence quotes (~30-40% token cut) and sample at temp 0.7; the
  provider-switch integration test from the old backlog now exists (`test_settings_api.py`).
- **Aux model split clarified (v2.3)** — the aux abstraction now defaults to the same single local
  model in local mode (measured: the 30b-a3b MoE decodes as fast as a dense 4B, so a local SLM split
  bought nothing) and keeps `openai_aux_model` for API mode. [ADR-0003](docs/adr/0003-dual-model-aux-routing.md)
  is superseded by [ADR-0010](docs/adr/0010-local-by-default.md) on this point.
